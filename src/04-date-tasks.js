...
function timeSpanToString(startDate, endDate) {
  ...
  return time.toISOString().substring(11, time.toISOString().length - 1);
}
...
function angleBetweenClockHands(date) {
  ...
  let hour = (date.getUTCHours() % 12) * 30;
  const min = date.getUTCMinutes() / 5;
  hour += (30 * min) / 12;
  let angle = Math.abs(hour - min * 30);

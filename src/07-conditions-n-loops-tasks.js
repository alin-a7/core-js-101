...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${firstBracket}${a < b ? a : b}, ${a < b ? b : a}${lastBrecket}`;
}
...
function isCreditCardNumber(ccn) {
  const cifer = ccn.toString().split('').map((x, i, arr) => {
    let res = x;
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (open.includes(str[i])) {
      ...
      if (open.indexOf(topEl) === closed.indexOf(str[i])) {
        stack.pop();
      }
    }
  }
...
function getCommonDirectoryPath(pathes) {
  const min = pathes.map((x) => x.length).sort((a, b) => a - b)[0];
  for (let i = 0; i < min; i += 1) {
    if (!pathes.every((x) => x[i] === pathes[1][i])) {
      let res = pathes[1].substring(0, i);
      if (res.length > 1) res = `${res.split('/').slice(0, -1).join('/')}/`;
      return res;
    }
  }
  return '';
...
function evaluateTicTacToePosition(position) {
  if (
    ...
    if (position[0][0] === 'X' || position[0][0] === '0') return position[0][0];
  }
  if (position[1][0] === position[1][1] && position[1][1] === position[1][2]) {
    if (position[1][0] === 'X' || position[1][0] === '0') return position[1][0];
  }
  if (position[0][1] === position[1][1] && position[1][1] === position[2][1]) {
    if (position[0][1] === 'X' || position[0][1] === '0') return position[1][1];
  }
  if (position[0][2] === position[1][1] && position[1][1] === position[2][0]) {
    if (position[0][2] === 'X' || position[0][2] === '0') return position[1][1];
  }
  if (position[2][0] === position[2][1] && position[2][0] === position[2][2]) {
    if (position[2][0] === 'X' || position[2][0] === '0') return position[2][0];
  }
  if (position[0][2] === position[1][2] && position[0][2] === position[2][2]) {
    if (position[0][2] === 'X' || position[0][2] === '0') return position[0][2];
  }
  return undefined;
}
